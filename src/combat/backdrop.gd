...
func _ready() -> void:
	# Grass
	spawn_random(Vector2i(-30, -20), Vector2i(30, 20), Vector2i(40, 40), Vector2(1.0, 1.0), $GrassMM, 0.3)

	# Bush
	spawn_random(Vector2i(-30, -15), Vector2i(30, 20), Vector2i(10, 10), Vector2(1.0, 1.0), $BushMM, 0.7)


func spawn_random(from: Vector2i, to: Vector2i, partitions: Vector2i, overflow: Vector2, multimesh: MultiMeshInstance3D, scale_min: float) -> void:
	...
	for x: int in x_range:
		for y: int in y_range:
			...
			xform = xform.scaled_local(Vector3.ONE * randf_range(scale_min, 1.0));
